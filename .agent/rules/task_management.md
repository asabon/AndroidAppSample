...
3. **作業用ブランチの作成**:
   - `gh issue develop [Issue番号] --name [Issue番号]-[任意の名前]` を使用して、GitHub 上で Issue に関連付けられたブランチを作成してください。
   - **命名規則**: 必ずブランチ名の先頭に Issue 番号を付与してください（例: `78-refine-rules`）。これにより、他ブランチとの重複を防ぎ、該当 Issue を特定しやすくします。
...
### 1. 完了報告（Antigravity）
- `task.md` を更新し、GitHub Issue 内のすべてのチェックボックスがチェックされていることを確認してください。
...
- **PR の構成ルール**:
  - **タイトル**: 変更内容が伝わる簡潔なタイトルにする（例: `[TASK] XXXの実装`）。
  - **説明**: 関連する Issue 番号（`Closes #XX`）を必ず含める。
  - **ラベル付与**: Release Drafter が正しくカテゴリ分けとバージョン推論を行えるよう、適切なラベル（`enhancement`, `bug`, `chore`, `minor`, `patch` 等）を必ず付与してください。
